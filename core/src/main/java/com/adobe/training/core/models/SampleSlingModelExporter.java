...
@Model(adaptables = Resource.class, resourceType = "acs-samples/components/content/sling-model", defaultInjectionStrategy = DefaultInjectionStrategy.OPTIONAL
...
@Exporter(name = "jackson", extensions = "json", options = {
		 ...
		 * http://static.javadoc.io/com.fasterxml
		 * .jackson.core/jackson-databind/2.8
		 * .5/com/fasterxml/jackson/databind/MapperFeature.html - Serialization
		 * Features:
		 * http://static.javadoc.io/com.fasterxml.jackson.core/jackson-
		 * databind/2.8
		 * .5/com/fasterxml/jackson/databind/SerializationFeature.html
		 */
...
public class SampleSlingModelExporter {
	...
	// Since the Default inject strategy is OPTIONAL (set on the @Model), we can
	// mark injections as @Required. @Optional can be used if the default
	...
	// before the Model object is returned for use.
	private void init() {
		// Note that @PostConstruct code will always be executed on Model
		// instantiation.
		// If the work done in PostConstruct is expensive and not always used in
		// the consumption of the model, it is
		// better to lazy-execute the logic in the getter and persist the result
		// in model state if it is requested again.
		page = resourceResolver.adaptTo(PageManager.class).getContainingPage(
				resource);
	...
	/**
	 * This getter wraps business logic around how an logic data point (title)
	 * is represented for this resource.
	 *
	...
	// @JsonIgnore is a Jackson Annotation specific to this field that prevents
	// this field from being serialized into the exported JSON.
	// For a list of Jackson Annotations see
	...
	/**
	 * @return the resource path to this content. Does not include the
	 *         extension.
	 */
	...
	// @JsonProperty is a Jackson Annotation specific to this field defines the
	// JSON Property name to expose this method as.
	// For a list of Jackson Annotations see
	// https://github.com/FasterXML/jackson-annotations/wiki/Jackson-Annotations
